    ...
    <div className="pointer-events-none fixed bottom-4 right-4 z-[100] flex w-full max-w-sm flex-col gap-3">
      {toasts.map((t) => (
        <div
          key={t.id}
          role="alert"
          className="toast-in pointer-events-auto flex w-full items-center rounded-lg bg-white p-4 text-gray-500 shadow ring-1 ring-black/5"
        >
          <div
          ...
          <button
            type="button"
            onClick={() => remove(t.id)}
            className="-mx-1.5 -my-1.5 ml-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-900 focus:ring-2 focus:ring-gray-300"
            aria-label="Đóng"
          >
