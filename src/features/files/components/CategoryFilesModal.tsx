...
interface CategoryFilesModalProps {
  category: { idCategory: string; categoryName: string } | null;
  onClose: () => void;
}

export function CategoryFilesModal({
  category,
  onClose,
}: CategoryFilesModalProps) {
  const idCategory = category?.idCategory;
  const { data, isLoading, isError } = useFilesByCategory(idCategory);
  const upload = useUploadFilesToCategory();
  ...
  const handleFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = "";
    if (!files.length || !idCategory) return;
    upload.mutate({ idCategory, files });
  };
      ...
      >
          ...
          <div className="relative z-10 flex items-center justify-between gap-2">
            {!viewing ? (
              <div className="min-w-0">
                ...
                <p className="text-xs text-white/70">{count} file</p>
              </div>
            ) : (
              <button
                type="button"
                onClick={() => setViewing(null)}
                className="rounded-full bg-white/15 px-3 py-2 text-sm font-medium text-white ring-1 ring-white/25 hover:bg-white/25"
              >
                ← Quay lại
              </button>
            )}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-6">
          {viewing ? (
            ...
            <EmptyState icon={FiFile} text="Danh mục này chưa có file nào." />
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {data!.map((f) => {
